...
■ 브리프 구조 (반드시 아래 태그 형식으로 작성):

executive_brief 값은 반드시 다음 태그로 시작하는 섹션들로 구성하세요:

[긴급 대응] 경쟁사 수주, 규제 변화, 파트너 이슈 등 즉시 대응이 필요한 사안. 해당 사안이 없으면 이 태그 자체를 생략.
[시장 시그널] 오늘 기사에서 포착된 가장 중요한 시장 변화 1-2개. 숫자/기업명/금액을 반드시 포함.
[기회 포착] ACRYL이 활용할 수 있는 구체적 사업 기회. GPUBASE/AGENTBASE/NADIA 중 해당 제품명 명시.
[주간 맥락] 이번 주 전체 흐름에서 오늘 기사가 갖는 의미. 반복 트렌드가 있으면 언급.

■ 예시 출력:
{
  "executive_brief": "[시장 시그널] CoreWeave가 NVIDIA B200 1만대 규모 계약을 체결했다. GPU-as-a-Service 시장이 2026년 $47B로 성장 전망이다. [기회 포착] GPUBASE의 멀티테넌트 GPU 관리 기능이 CoreWeave 대비 차별점이다. 공공 AI 인프라 사업 입찰에 해당 벤치마크를 활용해야 한다. [주간 맥락] 이번 주 GPU 클라우드 관련 기사가 12건으로 전주 대비 2배 증가했다. 시장 과열 신호로 해석된다."
}

■ 작성 규칙:
- 반드시 [시장 시그널], [기회 포착] 태그는 포함. [긴급 대응]은 해당 시만, [주간 맥락]은 선택.
